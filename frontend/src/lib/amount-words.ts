...
function under1000(n: number): string {
  ...
  const rest = n % 100;
  let prefix = hundreds === 1 ? "cent" : `${under20(hundreds)} cent`;
...
function convertInteger(n: number): string {
  ...
  for (const scale of scales) {
    if (n >= scale.value) {
      ...
      let head =
        scale.label === "mille" && count === 1
          ? "mille"
          : `${convertInteger(count)} ${scale.label}${count > 1 && scale.label !== "mille" ? "s" : ""}`;
...
export function amountToFrenchWords(amount: number): string {
  ...
  const currency = rounded <= 1 ? "dirham" : "dirhams";
  return `${capitalize(convertInteger(rounded))} ${currency}`;
}
